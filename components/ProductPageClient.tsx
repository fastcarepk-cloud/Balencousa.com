...
export default function ProductPageClient({ product, reviews: initialReviews }: ProductPageClientProps) {
            ...
            <div className="space-y-4 md:space-y-6">
              ...
              <div className="flex items-center space-x-4">
                <div className="flex items-center space-x-1">
                  {[...Array(5)].map((_, i) => (
                    <Star
                      key={i}
                      className={`w-4 h-4 md:w-5 md:h-5 ${i < Math.floor(product.rating) ? "fill-yellow-400 text-yellow-400" : "text-gray-300"}`}
                    />
                  ))}
                  <span className="text-base md:text-lg font-medium text-gray-900 ml-2">{product.rating}</span>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-sm md:text-base text-gray-600">Duration: {product.duration}</span>
                </div>
                <div className="flex items-center space-x-2">
              ...
              <div className="bg-gray-50 p-4 md:p-6 rounded-xl">
                <div className="flex items-center space-x-3 md:space-x-4 mb-3 md:mb-4">
                  <span className="text-2xl md:text-4xl font-bold text-gray-900">RS {product.price}</span>
                  {product.originalPrice > product.price && (
                    <span className="text-lg md:text-2xl text-gray-500 line-through">RS {product.originalPrice}</span>
                  )}
                  {product.discount > 0 && (
